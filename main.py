...
def check_key(auth: Optional[str]):
    ...
    if expected is None:
        return  # no auth enforced if API_KEY not set
...
def regime_today():
    with db_conn() as conn, conn.cursor() as cur:
        ...
        row = cur.fetchone()
    if row:
        return {"date": str(row[0]), "regime": row[1], "confidence": float(row[2]) if row[2] is not None else None}
...
def scores_top10():
    with db_conn() as conn, conn.cursor() as cur:
        cur.execute("""
            ...
            select s.symbol, s.score, coalesce(s.rank, row_number() over(order by s.score desc)) as rnk, s.reasons
            from score_daily s join latest l on s.asof = l.asof
            order by rnk asc, s.score desc
            limit 10
        """)
        rows = cur.fetchall()
    if not rows:
    ...
    items = []
    for sym, score, rnk, reasons in rows:
        items.append({"symbol": sym, "score": float(score), "rank": rnk, "why": reasons if reasons else []})
    return {"asof": "latest", "items": items}
...
def signals_recent(limit: int = 50):
    ...
    for (ts, sym, trig, entry, stop, tp1, tp2, regime, score, status) in rows:
        items.append({
            ...
            "regime": regime, "score": float(score) if score is not None else None,
            "status": status
        })
    return {"items": items}
...
def job_hygiene(authorization: Optional[str] = Header(None)):
    ...
    today = dt.date.today()
    # demo fields; you can replace with real calculations later
    payload = {
        "btc_trend": "UP", "eth_trend": "UP",
        "breadth_pct": 62.0, "stables_flow_pct": -0.4,
        "regime": "RISK_ON", "confidence": 0.63
    }
    with db_conn() as conn, conn.cursor() as cur:
        cur.execute("""
            insert into market_state_daily (asof, btc_trend, eth_trend, breadth_pct, stables_flow_pct, regime, confidence)
        ...
        """, (today, payload["btc_trend"], payload["eth_trend"], payload["breadth_pct"],
              payload["stables_flow_pct"], payload["regime"], payload["confidence"]))
        ...
        for sym, score, rnk, why in top:
            cur.execute("""
                ...
                on conflict (asof, symbol) do update
                  set score=excluded.score, reasons=excluded.reasons, rank=excluded.rank
            """, (today, sym, score, psycopg.types.json.Json(why), rnk))
...
def job_trigger4h(authorization: Optional[str] = Header(None)):
    ...
    sample = [
        (now, "SOLUSDT", "sweep→CHOCH→retest", 178.2, 169.5, 189.0, 198.0, "RISK_ON", 78.4),
        (now, "INJUSDT",  "AVWAP reclaim",      31.10,  28.90,  34.50,  37.00, "RISK_ON", 75.1),
    ]
    with db_conn() as conn, conn.cursor() as cur:
        for ts, sym, trig, entry, stop, tp1, tp2, regime, score in sample:
            cur.execute("""
                insert into signal_4h (ts, symbol, trigger, entry, stop, tp1, tp2, regime, score)
